...
func main() {
    ...
    startFn := func(event *docker.APIEvents) error {
		...
		sendToCWEvents(configuration.Region,`{ "dockerId": "` + event.ID + `","TaskArn":"` + taskArn + `" }`, "Task Started", configuration.Hostname, configuration.Hostname )
    ...
    stopFn := func(event *docker.APIEvents) error {
        ...
        taskArn := getTaskArn(event.ID)
		sendToCWEvents(configuration.Region, `{ "dockerId": "` + event.ID + `","TaskArn":"` + taskArn + `" }`, "Task Stopped", configuration.Hostname, configuration.Hostname )
